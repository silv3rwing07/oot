    ...
    for (i = 0; i < D_80130578[D_801333CC][bankId]; i++) {
        ...
        if (bankIndex != 0xFF) {
            ...
            if (entry->unk_2A == 2) {
                ...
                Audio_SendMsg_s8(AUDIO_CMD(0x06, 0x02, D_8016E260, 0), 1);
                Audio_SendMsg_s8(AUDIO_CMD(0x06, 0x02, D_8016E260, 4), entry->unk_28 & 0xFF);
                if (D_80130570[bankId] != 0) {
                    Audio_SendMsg_s8(AUDIO_CMD(0x06, 0x02, D_8016E260, 5), (entry->unk_28 & 0x100) >> 8);
                }
...
void func_800F87A0(u8 bankId) {
    ...
    while (bankIndex != 0xFF) {
        entry = &gSoundBanks[bankId][bankIndex];
        if (entry->unk_2A >= 3) {
            Audio_SendMsg_s8(AUDIO_CMD(0x06, 0x02, entry->unk_2E, 0), 0);
        }
        if (entry->unk_2A != 0) {
            func_800F7B54(bankId, bankIndex);
        }
